...
// RNG untuk menghasilkan angka acak dari 1 hingga jumlahDokter
int rng_dokter_acak(int jumlahDokter) {
    if (jumlahDokter <= 0) return -1;                // Validasi input
    return (rand() % jumlahDokter) + 1;              // Hasilkan angka antara 1 hingga jumlahDokter
}

void inisialisasiPelanggaranShift(Dokter* dokterUtama){
    while(dokterUtama != NULL){
        dokterUtama -> pelanggaran = 0;
        dokterUtama = dokterUtama -> next;
    }
}

void totalPelanggaran(Dokter* dokterUtama){
    int total_pelanggaran;
    while (dokterUtama != NULL){
        total_pelanggaran += dokterUtama -> pelanggaran;
        dokterUtama = dokterUtama -> next;
    }
    printf("total pelanggaran yang terjadi saat pembuatan jadwal: %d pelanggaran.\n", total_pelanggaran);
}
...
Dokter* salin_dokterSementara(Dokter** dokterHeadSementara, Dokter** dokterTailSementara, Dokter* dokterUtama) {
    ...
    // Salin ID
    dokterSementara->id = dokterUtama->id;
...
Jadwal* buat_jadwal_hari(Dokter* dokter){
    ...
    inisialisasiPelanggaranShift(dokterUtama);
    dokterUtama = head;
    ...
    for(i = 1; i <= total_hari; i++){
        ...
        for(j = 0; j <= 2; j++){
            int idDokterAcak = rng_dokter_acak(jumlah_dokter);
            int pilih;
            ...
            if((pilih == 0)){
                    ...
                    for(k = 0; k <= shift_sisa_pelanggaran; k++){
                        if(k == 0){ // Ambil Dokter Anak
                            while(dokterUtama != NULL){
                                if(cekAnak(dokterUtama) && !sudahAmbilShift(dokterUtama, arrayIDSHift) && cekShiftSisa(dokterUtama)){
                                    // Masukin ke struct dokter pakai fungsi nashwa
                                    salin_dokterSementara(&dokterHeadSementara, &dokterTailSementara, dokterUtama); 
                                    dokterUtama -> pelanggaran += 1;
                                    pilih = 1;
                                    dokterUtama -> max_shift -= 1;
                                    arrayIDSHift[idDokterTersedia++] = dokterUtama -> id;
                                    dokterUtama = dokterUtama -> next;
                            ...
                            while(dokterUtama != NULL){
                                if(cekUmum(dokterUtama) && !sudahAmbilShift(dokterUtama, arrayIDSHift) && cekShiftSisa(dokterUtama)){
                                    // Masukin ke struct dokter pakai fungsi nashwa
                                    salin_dokterSementara(&dokterHeadSementara, &dokterTailSementara, dokterUtama);
                                    dokterUtama -> pelanggaran += 1;
                                    pilih = 1;
                            ...
                            while(dokterUtama != NULL){
                                if(cekBedah(dokterUtama) && !sudahAmbilShift(dokterUtama, arrayIDSHift) && cekShiftSisa(dokterUtama)){
                                    // Masukin ke struct dokter pakai fungsi nashwa
                                    salin_dokterSementara(&dokterHeadSementara, &dokterTailSementara, dokterUtama);
                                    dokterUtama -> pelanggaran += 1;
                                    pilih = 1;
                                    ...
                                    int ketemuIDAcak = 0;
                                    while(!ketemuIDAcak){
                                        if(dokterUtama -> id == idDokterAcak){
                                            ketemuIDAcak = 1;
                                        }
                                        else{
                                            dokterUtama = dokterUtama -> next;
                            ...
                            while(dokterUtama != NULL){
                                if(!sudahAmbilShift(dokterUtama, arrayIDSHift) && cekShiftSisa(dokterUtama)){
                                    // Masukin ke struct dokter pakai fungsi nashwa
                                    salin_dokterSementara(&dokterHeadSementara, &dokterTailSementara, dokterUtama);
                                    dokterUtama -> pelanggaran += 1;
                                    pilih = 1;
